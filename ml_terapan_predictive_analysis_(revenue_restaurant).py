...
bins = [0, 20000, 40000, 60000, df['Social Media Followers'].max()]
labels = ['Sedikit (0-20k)', 'Sedang (20k-40k)', 'Banyak (40k-60k)', 'Sangat Banyak (>60k)']

df['Kategori_Pengikut_Medsos'] = pd.cut(df['Social Media Followers'], bins=bins, labels=labels, include_lowest=True)

plt.figure(figsize=(8,6))
sns.barplot(data=df, x='Kategori_Pengikut_Medsos', y='Revenue', estimator='mean')
...
bins = [29.939, 45.0, 60.0, 75.0, 90.0]
labels = [
    'Kecil (29–45 kursi)',
    'Sedang (45–60 kursi)',
    'Cukup besar (60–75 kursi)',
    'Sangat besar (75–90 kursi)'
]

df['Kategori_Kapasitas_TempatDuduk'] = pd.cut(
    df['Seating Capacity'],
    bins=bins,
    labels=labels,
    include_lowest=True
...
plt.figure(figsize=(10,6))
sns.barplot(data=df, x='Kategori_Kapasitas_TempatDuduk', y='Revenue', estimator='mean')
...
min_price = df['Average Meal Price'].min()
q1 = df['Average Meal Price'].quantile(0.25)
q2 = df['Average Meal Price'].quantile(0.50)
q3 = df['Average Meal Price'].quantile(0.75)
max_price = df['Average Meal Price'].max()

bins = [min_price, q1, q2, q3, max_price]
labels = [
...
df['Kategori_Harga_Makanan'] = pd.cut(df['Average Meal Price'], bins=bins, labels=labels, include_lowest=True)

plt.figure(figsize=(10,6))
sns.barplot(data=df, x='Kategori_Harga_Makanan', y='Revenue', estimator='mean', errorbar='sd')
...
"""Grafik menunjukkan bahwa semakin tinggi rata-rata harga makanan, semakin besar rata-rata pendapatan restoran. Restoran dengan harga sangat mahal (>59\$) mencatatkan pendapatan tertinggi, mendekati Rp900.000. Hal ini mengindikasikan bahwa strategi harga tinggi dapat meningkatkan revenue, meskipun efektivitasnya tetap bergantung pada daya beli dan preferensi pasar.
...
bins = [0, 20, 40, 60, df['Weekend Reservations'].max()]
labels = ['Sedikit', 'Sedang', 'Ramai', 'Sangat Ramai']

df['Weekend_Reservations_Category'] = pd.cut(df['Weekend Reservations'], bins=bins, labels=labels, include_lowest=True)

plt.figure(figsize=(8,6))
sns.pointplot(data=df, x='Weekend_Reservations_Category', y='Revenue', estimator='mean')
